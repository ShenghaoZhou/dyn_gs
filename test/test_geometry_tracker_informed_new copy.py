...
class Config:
    ...
    # Keyframe Parameters
    kf_disparity_thresh: float = 30.0
    kf_min_interval: int = 5
    kf_max_interval: int = 30
    ...
    skip_pnp: bool = False
    informed_thresh: float = 20.0 # More conservative
    guess_type: str = "GT-Cam+CV-Obj" # Use CV-Obj for better priors
...
def main(cfg: Config):
    ...
    for i in tqdm(range(1, cfg.n_frames), desc="Tracking"):
        ...
        # 3. Adaptive Keyframe Selection using DIS flow disparity
        curr_gray = cv2.cvtColor(fd["image"], cv2.COLOR_RGB2GRAY)
        flow_kf_curr = dis.calc(last_kf_gray, curr_gray, None)
        mask_curr = fd["mask"] > 0
        avg_disparity = np.mean(np.linalg.norm(flow_kf_curr, axis=-1)[mask_curr]) if np.any(mask_curr) else 0
        ...
        if is_kf:
            print(f"[Keyframe] Frame {idx}: Disp {avg_disparity:.1f}px, Recovered {recovered}, Active {len(active_tids)}")
