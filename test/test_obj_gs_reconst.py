...
class Config:
    ...
    use_pgsr: bool = True
    no_gs: bool = False # Bypass GS operations and use GT directly
...
def main(cfg: Config):
    rr.init("obj_gs_reconst")
    rr.connect_grpc("rerun+http://128.175.109.248:9876/proxy")
    
    if cfg.no_gs:
        cfg.use_gt_depth = True # Force GT depth if no_gs is requested
    ...
    if cfg.use_gt_depth:
        ...
        if depth_0 is None or mask_0 is None:
            # Fallback to GT if model_infer is missing for some reason
            depth_0 = frame_0.get("depth")
            mask_0 = frame_0["obj_mask"]
    ...
    if not cfg.no_gs:
        ...
        # gs_params.scales.requires_grad = True
        # gs_params.colors.requires_grad = True
    ...
    keyframes = [] # List of MiniCam objects and images
    ...
    for idx in tqdm(range(len(data_loader))):
        ...
        # GS Operations
        render_image, render_depth, render_normal = None, None, None
        if not cfg.no_gs:
            # Optimization steps for current frame
            for step in range(cfg.num_steps_per_frame):
                optimizer.zero_grad()
                
                # Render (using exp(scales) and sigmoid(opacity))
                ...
                # Mask loss (penalize rendering outside the object mask)
                ...
                if cfg.use_pgsr:
                    loss_single = compute_single_view_loss(
                        render_normal, render_depth, image_t, cam, 
                        weight=cfg.single_view_weight, mask=mask_t
                    )
                
                # Multi-view PGSR loss
                loss_multi = torch.tensor(0.0, device=device)
                if cfg.use_pgsr and len(keyframes) > 0:
                    # Sample a keyframe from sliding window
                    ...
                    grid_y, grid_x = torch.meshgrid(
                        torch.arange(cam.height, device=device), 
                        torch.arange(cam.width, device=device), 
                        indexing='ij'
                    )
                    pixels = torch.stack([grid_x, grid_y], dim=-1).float().reshape(-1, 2)
                    
                    # Filter pixels by mask
                    ...
                    if len(mask_indices) > 0:
                        ncc, ncc_mask = compute_multi_view_loss(
                            image_t.mean(0).unsqueeze(0), kf_image_gray.unsqueeze(0), 
            ...
            # Densification (every densify_every frames)
            if idx > 0 and idx % cfg.densify_every == 0:
                with torch.no_grad():
                    ...
                    if depth_curr is not None:
                        ...
                        # Adjust depth scale using current GS render in overlapping area
                        if not cfg.use_gt_depth:
                            ...
                            if overlap_mask.sum() > 100:
                                ...
                                print(f"Adjusted depth scale by {median_scale:.4f}")
                                
                        # Laplacian-based densification (following scene_model.py)
                        init_proba = get_lapla_norm(image_t, disc_kernel)
                        penalty = get_lapla_norm(render_image, disc_kernel)
                        
                        # Sample mask: high gradient in GT but not in render, and closer than render
                        sample_mask = (torch.rand_like(init_proba) < (init_proba - penalty) * 2.0)
                        sample_mask = sample_mask & mask_t & (depth_curr_t > 0)
                        
                        # Also consider depth: if GT is closer than render, or render is empty, it might be a new surface
                        # render_depth is [1, H, W]. 0 means background/empty.
                        ...
                        if sample_mask.sum() > 0:
                            # Subsample high error pixels
                            y, x = torch.where(sample_mask)
                            if len(y) > 2000: # Limit per frame
                                perm = torch.randperm(len(y), device=device)[:2000]
                                y, x = y[perm], x[perm]
                            
                            # Unproject selected pixels to camera space
                            z = depth_curr_t[y, x]
                            ...
                            # Get normals for newly added points
                            # We unproject the whole image to get better normal estimation
                            ...
                            # Flip to point towards camera and normalize
                            normals_c = -normals_c[0] # [3, H, W]
                            normals_c = F.normalize(normals_c, dim=0)
                            
                            # Sample normals at new point locations
                            new_normals_c = normals_c[:, y, x].permute(1, 0) # [N, 3]
                            ...
                            # Initialize new GS parameters
                            new_means = pts_o
                            new_colors = RGB2SH(image_t[:, y, x].permute(1, 0))
                            
                            # Initialize quaternions from normals
                            new_quats = build_rotation_from_normal(new_normals_o)
                            
                            # Initialize scales using Laplacian (following scene_model.py logic)
                            # scales = 1 / sqrt(init_proba) * dist / f
                            sampled_init_proba = init_proba[y, x].clamp_min(1e-6)
                            # Use pixel-space size first, then convert to metric
                            # Increase minimum pixel size to avoid holes (from 1.0 to 2.0)
                            ...
                            new_opacity = torch.log(new_opacity / (1 - new_opacity)) # inverse sigmoid
                            
                            # Append to existing GS
        ...
        if idx % cfg.kf_every == 0:
            ...
            if len(keyframes) > 100: # Optional limit
                keyframes.pop(0)
        ...
        if not cfg.no_gs:
            with torch.no_grad():
                ...
                dist = means_c[:, 2].clamp_min(0.01) # Avoid division by zero
                
                curr_scales = torch.exp(gs_params.scales)
                max_scales = curr_scales.max(dim=-1)[0]
                # Use focal length from K
                f = cam.K[0, 0]
                screen_size = f * max_scales / dist
                
                # Pruning criteria: opacity > th and screen_size not too large
                ...
                if valid_mask.sum() == 0:
                    print("Warning: All Gaussians would be pruned. Keeping all.")
                ...
                if n_before != n_after:
                    print(f"Pruned GS: {n_before} -> {n_after}")
                    
                # Re-initialize optimizer if anything changed
                if n_before != n_after:
                    optimizer = torch.optim.Adam([
                        {"params": [gs_params.means], "lr": 0.001},
                        {"params": [gs_params.quats], "lr": 0.001},
        ...
        if idx % 1 == 0:
            ...
            if render_image is not None:
                rr.log("render", rr.Image(render_image.permute(1, 2, 0).detach().cpu().numpy().clip(0, 1)))
            if render_normal is not None:
                rr.log("normal", rr.Image(((render_normal.permute(1, 2, 0).detach().cpu().numpy() + 1) / 2 * 255).astype(np.uint8)))
            
            # Log points and cameras in Object Frame
            if not cfg.no_gs:
                with torch.no_grad():
                    # Convert SH to RGB for Rerun pointcloud visualization
    ...
    for idx in tqdm(range(len(data_loader)), desc="Fusing"):
        ...
        if not cfg.no_gs:
            with torch.no_grad():
                ...
                # render_image is [3, H, W], needs [H, W, 3] and uint8
                color_np = (render_image.permute(1, 2, 0).cpu().numpy().clip(0, 1) * 255).astype(np.uint8)
                # render_depth is [1, H, W], needs [H, W] and float32
                depth_np = render_depth[0].cpu().numpy()
        else:
            # Use GT data
            color_np = image
            depth_np = frame.get("depth")
            if depth_np is None:
                # print(f"Warning: GT Depth not found for frame {idx}")
                continue
            # Apply mask to depth for object reconstruction
            mask = frame["obj_mask"]
        ...
        # Open3D integrate expects world-to-camera matrix. 
        # Since our volume is in Object Frame, T_CO is exactly what we need.
