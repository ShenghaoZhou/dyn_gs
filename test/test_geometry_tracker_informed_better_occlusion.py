...
def main(cfg: Config):
    ...
    # History for informed guess (using estimates to avoid cheating)
    ...
    for i in tqdm(range(1, cfg.n_frames), desc="Tracking"):
        ...
        if cfg.guess_type == "GT-Cam+CV-Obj":
            ...
            # Uses predicted history instead of GT history to avoid cheating
        ...
        # Update trajectories (camera in object frame)
        T_CO_est = geo_tracker.poses[idx]
        T_OC_est = np.linalg.inv(T_CO_est)
        traj_obj_est_C.append(T_OC_est[:3, 3])
        
        # Update estimated object-to-world history for next frame's CV guess
        # T_WO = inv(T_CW) @ T_CO
        T_WO_est = np.linalg.inv(fd["T_CW_gt"]) @ T_CO_est
