...
from gs_dyn_obj.gs_rendering import render_3dgs
...
class Config:
    ...
    use_pgsr: bool = False # 3DGS doesn't produce normals easily for PGSR loss
...
def main(cfg: Config):
    ...
    # For 3DGS, we want 3 scales. Let's make it slightly flat by making the 3rd dimension smaller initially.
    scales_3d = torch.cat([gs_params.scales, gs_params.scales[:, :1] * 0.1], dim=-1)
    gs_params.scales = torch.log(scales_3d.clamp_min(1e-6))
    ...
    for idx in tqdm(range(len(data_loader))):
        ...
        for step in range(cfg.num_steps_per_frame):
            ...
            # Render 3DGS (using exp(scales) and sigmoid(opacity))
            render_image, render_depth, render_normal, render_alpha = render_3dgs(
            ...
            # Single-view loss (requires normal, so typically disabled for 3DGS unless we derive normal from depth)
            loss_single = torch.tensor(0.0, device=device)
            if cfg.use_pgsr:
                # If we really want to use it, we'd need to compute render_normal from render_depth
                # but for 3DGS it's better to disable it or use a different surface constraint.
            ...
            if cfg.use_pgsr and len(keyframes) > 0:
                # Multi-view LNCC also requires normals for homography.
                # Disabled by default for 3DGS.
        ...
        if idx > 0 and idx % cfg.densify_every == 0:
            with torch.no_grad():
                render_image, render_depth, _, render_alpha = render_3dgs(
                    gs_params.means, F.normalize(gs_params.quats), torch.exp(gs_params.scales),
                    gs_params.colors, torch.sigmoid(gs_params.opacity),
                    viewmat=T_CO_t, K=cam.K,
                    width=cam.width, height=cam.height
                ...
                if depth_curr is not None:
                    ...
                    if sample_mask.sum() > 0:
                        ...
                        # Initialize 3D scales, 3rd dimension smaller for surface initialization
                        new_scales_lin = new_sizes.view(-1, 1).repeat(1, 3)
                        new_scales_lin[:, 2] *= 0.1
                        new_scales = torch.log(new_scales_lin.clamp(1e-6, 1e6))
        ...
        if idx % 1 == 0:
            ...
            # No normal logging for 3DGS as it's empty
    ...
    for idx in tqdm(range(len(data_loader)), desc="Fusing"):
        ...
        with torch.no_grad():
            render_image, render_depth, _, _ = render_3dgs(
                gs_params.means, F.normalize(gs_params.quats), torch.exp(gs_params.scales),
                gs_params.colors, torch.sigmoid(gs_params.opacity),
