...
def main(cfg: Config):
    rr.init("test_ba_vs_pnp_orb", spawn=False)
    ...
    orb = cv2.ORB_create(nfeatures=cfg.max_orb_features)
    ...
    # Log Frame 0 GT points as static reference (Dense Cloud)
    f0 = frames[0]
    ...
    for f in frames:
        ...
        rr.log("world/camera_gt", rr.ViewCoordinates.RDF, static=True)
    ...
    kps0 = orb.detect(gray0, mask=frames[0]["mask"])
    orb_pts0 = np.array([kp.pt for kp in kps0], dtype=np.float32)
    tracks = {i: {"obs": {frames[0]["frame_idx"]: p}, "id": i} for i, p in enumerate(orb_pts0)}
    
    print("Tracking ORB with DIS flow (and optional KLT)...")
    for i in range(len(frames) - 1):
        ...
        if not valid_tids: 
            print(f"Warning: No valid tracks at frame {f_prev['frame_idx']}")
            break
        
        pts_prev = np.array([tracks[tid]["obs"][f_prev["frame_idx"]] for tid in valid_tids], dtype=np.float32)
        pts_next_dis = (pts_prev + interpolate_flow(flow, pts_prev)).astype(np.float32)
        
        if cfg.use_klt:
            # Refine DIS result with KLT
            # calcOpticalFlowPyrLK expects (N, 1, 2)
        ...
        mask_valid = (ix >= 0) & (ix < W) & (iy >= 0) & (iy < H)
        for j, tid in enumerate(valid_tids):
            if status[j] and mask_valid[j] and f_curr["mask"][iy[j], ix[j]] > 0:
                tracks[tid]["obs"][f_curr["frame_idx"]] = pts_next[j]

    full_tracks = {tid: t for tid, t in tracks.items() if len(t["obs"]) == cfg.window_size}
    print(f"Full tracks (ORB): {len(full_tracks)}")

    # Dense Point Cloud Tracking (for visualization only)
    print("Tracking dense grid for visualization...")
    dense_grid_pts = sample_grid_on_mask(frames[0]["mask"], cfg.grid_spacing)
    dense_tracks = {i: {"obs": {frames[0]["frame_idx"]: p}} for i, p in enumerate(dense_grid_pts)}
    for i in range(len(frames) - 1):
        f_prev, f_curr = frames[i], frames[i+1]
        gray_prev = cv2.cvtColor(f_prev["image"], cv2.COLOR_RGB2GRAY)
        gray_curr = cv2.cvtColor(f_curr["image"], cv2.COLOR_RGB2GRAY)
        flow = dis.calc(gray_prev, gray_curr, None)
        valid_tids = [tid for tid in dense_tracks if f_prev["frame_idx"] in dense_tracks[tid]["obs"]]
        pts_prev = np.array([dense_tracks[tid]["obs"][f_prev["frame_idx"]] for tid in valid_tids])
        pts_next = pts_prev + interpolate_flow(flow, pts_prev)
        ix, iy = np.round(pts_next[:, 0]).astype(int), np.round(pts_next[:, 1]).astype(int)
        mask_valid = (ix >= 0) & (ix < W) & (iy >= 0) & (iy < H)
        for j, tid in enumerate(valid_tids):
            if mask_valid[j] and f_curr["mask"][iy[j], ix[j]] > 0:
                dense_tracks[tid]["obs"][f_curr["frame_idx"]] = pts_next[j]
    ...
    print("\n--- Path C: Standard Relative Initialization (ORB) ---")
    ...
    # Final Dense Point Cloud Visualization (using Path A BA poses as reference for "Best Estimate")
    # T_WC_viz = T_WO0 @ T_OC0_gt @ inv(T_CiC0)
    print("Logging final dense cloud...")
    for f in frames:
        rr.set_time("frame", sequence=f["frame_idx"])
        T_CiC0 = poses_a[f["frame_idx"]]
        T_C0Ci = np.linalg.inv(T_CiC0)
        T_OCi = T_OC0_gt @ T_C0Ci
        T_WCi_viz = T_WO0 @ T_OCi
        
        valid_dense_tids = [tid for tid in dense_tracks if f["frame_idx"] in dense_tracks[tid]["obs"]]
        if not valid_dense_tids: continue
        
        pts_uv = np.array([dense_tracks[tid]["obs"][f["frame_idx"]] for tid in valid_dense_tids])
        # We need depth for these points. Use the depth map of the current frame
        d = np.array([f["depth"][int(round(uv[1])), int(round(uv[0]))] for uv in pts_uv])
        valid_d = d > 0.01
        ...
        # Sample colors from current image
        colors = f["image"][np.round(pts_uv[:, 1]).astype(int), np.round(pts_uv[:, 0]).astype(int)]
