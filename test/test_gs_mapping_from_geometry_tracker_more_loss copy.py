    ...
    for i in pbar:
        ...
        if idx % 5 == 0:
            with torch.no_grad():
                ...
                if sample_mask.any():
                    ...
                    new_gs = init_gs_from_tracker_points(pts_o.cpu().numpy(), (image_t[:, yy_v, xx_v].permute(1, 0)).cpu().numpy(), device)
                    for attr in ['means', 'quats', 'scales', 'colors', 'opacity']:
                        setattr(gs_params, attr, torch.nn.Parameter(torch.cat([getattr(gs_params, attr).data, getattr(new_gs, attr).data], dim=0)))
        ...
        for step in range(cfg.num_steps_per_frame):
            ...
            if cfg.use_mask_loss:
                ...
                loss_mask = F.l1_loss(render_alpha, target_mask.float()) * cfg.mask_loss_weight
            else:
            ...
            for kf in ref_keyframes:
                ...
                if cfg.use_mask_loss:
                    loss_mv += cfg.ref_frame_weight * cfg.mask_loss_weight * F.l1_loss(r_alpha, kf_mask.float())
                else:
