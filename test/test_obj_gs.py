...
def main(cfg: Config):
    ...
    for idx in tqdm(range(len(data_loader))):
        ...
        # Use current camera pose as given
        T_C_W_torch = torch.from_numpy(T_C_W).float().cuda()
        
        # Create noisy estimate for visualization only
        ...
        # Optimize pose change delta_T on top of current T_W_O
        T_W_O_est, losses = obj_gs.optimize_wrt_image(
        ...
        T_W_O_est_np = T_W_O_est.detach().cpu().numpy()
