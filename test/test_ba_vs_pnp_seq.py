...
class PathProcessor:
    ...
    def add_new_points_from_depth(self, frame_idx, image, mask, depth, K, T_CiC0, align=False):
        ...
        if align and self.tracks:
            ...
            # Project existing points into current frame
            z_est = []
            z_raw = []
            ...
            for tid, t in self.tracks.items():
                if frame_idx in t['obs']:
                    uv = t['obs'][frame_idx]
                    # Current point in C0: pt3d
                    # Current point in Ci: T_CiC0 @ pt3d
            ...
            if np.sum(valid) > 10:
                ...
                print(f"[{self.name}] Alignment failed (insufficient points), using raw depth.")
    ...
    def step(self, frame_idx, image, mask, depth, K, flow_prev_curr=None):
        if flow_prev_curr is not None:
            # Advect tracks
            prev_idx = self.keyframes[-1] if self.keyframes else frame_idx - 1 # This is simplified
            # Actually we should always advect from the immediately preceding frame
            # Let's assume frame_idx-1 is available
        ...
        # Filter track inliers
        inliers = set(np.array(active_tids)[info['inliers']])
        # for tid in active_tids:
        #     if tid not in inliers: del self.tracks[tid]['obs'][frame_idx]
...
def main(cfg: Config):
    ...
    for i in tqdm(range(cfg.window_size, cfg.n_frames)):
        ...
        # Step Path A
        path_a.step(curr_idx, f_curr["image"], f_curr["mask"], f_curr["depth"], f_curr["K"], flow)
        # Step Path B
