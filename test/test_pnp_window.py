...
class Config:
    ...
    # Debug Parameters
    stop_at: str = "ba" # Options: "init", "pnp", "ba"
...
def run_window_ba(frame_indices, poses, tracks, K_dict):
    ...
    track_params = {} # track_id -> pt3d (in C0 frame)
    relevant_tracks = []
    for track in tracks:
        win_obs = {f_idx: uv for f_idx, uv in track['obs'].items() if f_idx in frame_indices}
        if len(win_obs) >= 2:
            track_params[track['id']] = track['pt3d'].copy().astype(np.float64)
    ...
    for track, win_obs in relevant_tracks:
        ...
        for f_idx, uv in win_obs.items():
            ...
            uv_d = uv.astype(np.float64)
            cost = pycolmap.cost_functions.ReprojErrorCost('PINHOLE', uv_d)
...
def main(cfg: Config):
    ...
    def load_frame_data(frame_idx):
        ...
        T_WC_gt = np.load(data_dir / "extrinsics" / f"{stem}.npy")
        ...
        return {
            ...
            "T_WC_gt": T_WC_gt, "T_CW_gt": np.linalg.inv(T_WC_gt), "T_WO_gt": T_WO_gt, 
            "frame_idx": frame_idx
        }
    ...
    use_gt_flow = False # Set to True for debugging
    if use_gt_flow:
        print("DEBUG: Using GT flow for initialization")
        for i in range(len(frames) - 1):
            f_i, f_next = frames[i], frames[i+1]
            T_CiOi = np.linalg.inv(f_i["T_WC_gt"]) @ f_i["T_WO_gt"]
            T_CnextOnext = np.linalg.inv(f_next["T_WC_gt"]) @ f_next["T_WO_gt"]
            T_CnextCi = T_CnextOnext @ np.linalg.inv(T_CiOi)
            
            u_tmp = np.clip(np.round(obs_list[-1][:, 0]).astype(int), 0, W - 1)
            v_tmp = np.clip(np.round(obs_list[-1][:, 1]).astype(int), 0, H - 1)
            X_Ci = (np.linalg.inv(f_i["K"]) @ np.concatenate([obs_list[-1], np.ones((len(obs_list[-1]), 1))], axis=-1).T).T * f_i["depth_gt"][v_tmp, u_tmp][:, None]
            X_Cnext = (T_CnextCi[:3, :3] @ X_Ci.T).T + T_CnextCi[:3, 3]
            pts_next_h = (f_next["K"] @ X_Cnext.T).T
            pts_next = pts_next_h[:, :2] / pts_next_h[:, 2:]
            obs_list.append(pts_next)
    else:
        for i in range(len(frames) - 1):
            f_i, f_next = frames[i], frames[i+1]
            gray_i = cv2.cvtColor(f_i["image"], cv2.COLOR_RGB2GRAY)
            gray_next = cv2.cvtColor(f_next["image"], cv2.COLOR_RGB2GRAY)
            p1, st, err = cv2.calcOpticalFlowPyrLK(gray_i, gray_next, obs_list[-1], None)
            # For simplicity, just use p1. Ideally filter by st.
            obs_list.append(p1.reshape(-1, 2))
    ...
    # Report Statistics
    num_total = len(pts_start)
    num_inliers = info['num_inliers']
    print(f"Init RelPose: {num_inliers}/{num_total} inliers ({num_inliers/num_total:.2%})")

    # GT Reprojection Check
    X_C0_gt_real = (np.linalg.inv(f_start["K"]) @ np.concatenate([pts_start, np.ones((len(pts_start), 1))], axis=-1).T).T * f_start["depth_gt"][np.round(pts_start[:, 1]).astype(int), np.round(pts_start[:, 0]).astype(int)][:, None]
    T_C1W_gt = np.linalg.inv(f_end["T_WC_gt"])
    X_C1_gt_real = (T_C1W_gt[:3, :3] @ (f_end["T_WC_gt"][:3, :3] @ X_C0_gt_real.T + f_end["T_WC_gt"][:3, 3][:, None])).T # This is just X_C0 in world then to C1
    # Wait, simpler:
    T_C1C0_gt_world = np.linalg.inv(f_end["T_WC_gt"]) @ f_start["T_WC_gt"]
    X_C1_gt_world = (T_C1C0_gt_world[:3, :3] @ X_C0_gt_real.T).T + T_C1C0_gt_world[:3, 3]
    pts_end_gt_world_h = (f_end["K"] @ X_C1_gt_world.T).T
    pts_end_gt_world = pts_end_gt_world_h[:, :2] / pts_end_gt_world_h[:, 2:]
    
    # If the object is moving, this should be WRONG.
    # If the object is static, this should be RIGHT.
    print(f"Static-World Flow Error: {np.mean(np.linalg.norm(pts_end - pts_end_gt_world, axis=1)):.4f} px")
    T_C1O1_gt_tmp = np.linalg.inv(f_end["T_WC_gt"]) @ f_end["T_WO_gt"]
    T_C0O0_gt_tmp = np.linalg.inv(f_start["T_WC_gt"]) @ f_start["T_WO_gt"]
    T_C1C0_gt_tmp = T_C1O1_gt_tmp @ np.linalg.inv(T_C0O0_gt_tmp)
    
    X_C0_gt = (np.linalg.inv(f_start["K"]) @ np.concatenate([pts_start, np.ones((len(pts_start), 1))], axis=-1).T).T * d_s[:, None]
    X_C1_gt = (T_C1C0_gt_tmp[:3, :3] @ X_C0_gt.T).T + T_C1C0_gt_tmp[:3, 3]
    pts_end_gt_h = (f_end["K"] @ X_C1_gt.T).T
    pts_end_gt = pts_end_gt_h[:, :2] / pts_end_gt_h[:, 2:]
    
    dis_disp = np.linalg.norm(pts_end - pts_start, axis=1)
    gt_disp = np.linalg.norm(pts_end_gt - pts_start, axis=1)
    print(f"Mean DIS Displacement: {np.mean(dis_disp):.4f} px")
    print(f"Mean GT Displacement: {np.mean(gt_disp):.4f} px")
    ...
    # Initialize tracks with observations from start and end frames
    # Intermediate observations will be added if they are PnP inliers
    tracks = []
    ...
    if cfg.stop_at == "init":
        # Debug: Run PnP on the last frame using the relative-pose-initialized 3D points
        cam_pnp = {'model': 'PINHOLE', 'width': W, 'height': H, 'params': [f_end["K"][0, 0], f_end["K"][1, 1], f_end["K"][0, 2], f_end["K"][1, 2]]}
        res_pnp, info_pnp = poselib.estimate_absolute_pose(pts_e_in, initial_pts3d_C0, cam_pnp, ransac_opt, None)
        T_CiC0_pnp = np.eye(4)
        T_CiC0_pnp[:3, :3] = R.from_quat([res_pnp.pose.q[1], res_pnp.pose.q[2], res_pnp.pose.q[3], res_pnp.pose.q[0]]).as_matrix()
        T_CiC0_pnp[:3, 3] = res_pnp.pose.t
        
        # Log PnP Result for comparison
        T_WC_pnp = f_end["T_WO_gt"] @ T_OC0_gt @ np.linalg.inv(T_CiC0_pnp)
        rr.set_time("frame", sequence=f_end["frame_idx"])
        rr.log("world/init/camera_pnp", rr.Pinhole(image_from_camera=f_end["K"], width=W, height=H))
        rr.log("world/init/camera_pnp", rr.Transform3D(mat3x3=T_WC_pnp[:3, :3], translation=T_WC_pnp[:3, 3]))
        
        # Report Statistics
        num_total_pnp = len(pts_e_in)
        num_inliers_pnp = info_pnp['num_inliers']
        print(f"PnP Frame {f_end['frame_idx']} (Init Debug): {num_inliers_pnp}/{num_total_pnp} inliers ({num_inliers_pnp/num_total_pnp:.2%})")
        
        print(f"RelPose Translation Mag: {np.linalg.norm(res.pose.t):.4f}")
        print(f"PnP Translation Mag: {np.linalg.norm(res_pnp.pose.t):.4f}")
        
        # GT Translation Mag
        T_C1O1_gt = np.linalg.inv(f_end["T_WC_gt"]) @ f_end["T_WO_gt"] # T_C1,O1
        T_C0O0_gt = np.linalg.inv(f_start["T_WC_gt"]) @ f_start["T_WO_gt"] # T_C0,O0
        T_C1C0_gt_val = T_C1O1_gt @ np.linalg.inv(T_C0O0_gt) # T_C1,O1 * T_O0,C0
        print(f"GT Translation Mag: {np.linalg.norm(T_C1C0_gt_val[:3, 3]):.4f}")

        # Calculate reprojection error manually
        q = res_pnp.pose.q
        t = res_pnp.pose.t
        R_mat = R.from_quat([q[1], q[2], q[3], q[0]]).as_matrix()
        pts_3d_transformed = (R_mat @ initial_pts3d_C0.T).T + t
        K = f_end["K"]
        pts_2d_proj_h = (K @ pts_3d_transformed.T).T
        pts_2d_proj = pts_2d_proj_h[:, :2] / (pts_2d_proj_h[:, 2:] + 1e-8)
        inliers = np.array(info_pnp['inliers'])
        errs = np.linalg.norm(pts_2d_proj[inliers] - pts_e_in[inliers], axis=1)
        reproj_err = np.mean(errs)
        print(f"PnP Frame {f_end['frame_idx']} Reproj Error: {reproj_err:.4f} px")
        
        log_results("init", frames, poses_est, tracks, pts_O_gt, color_gt, T_OC0_gt, W, H)
        return

    # Intermediate PnP using sequential observations
    for i in range(1, cfg.window_size - 1):
        f_curr = frames[i]
        pts_2d = obs_list_in[i]
        u, v = np.round(pts_2d[:, 0]).astype(int), np.round(pts_2d[:, 1]).astype(int)
        valid = (u >= 0) & (u < W) & (v >= 0) & (v < H)
        if np.any(valid): valid[valid] &= (f_curr["mask"][v[valid], u[valid]] > 0)
        pts_2d_v, pts_3d_v = pts_2d[valid].astype(np.float64), initial_pts3d_C0[valid].astype(np.float64)
        ...
        T_CiC0 = np.eye(4)
        T_CiC0[:3, :3] = R.from_quat([res_abs.pose.q[1], res_abs.pose.q[2], res_abs.pose.q[3], res_abs.pose.q[0]]).as_matrix()
        T_CiC0[:3, 3] = res_abs.pose.t
        poses_est[f_curr["frame_idx"]] = T_CiC0
        inliers_abs, valid_indices = info_abs['inliers'], np.where(valid)[0]
        for idx_inlier in range(len(inliers_abs)):
            if inliers_abs[idx_inlier]: tracks[valid_indices[idx_inlier]]['obs'][f_curr["frame_idx"]] = pts_2d_v[idx_inlier]
    ...
    # BA
    run_window_ba([f["frame_idx"] for f in frames], poses_est, tracks, K_dict)
    log_results("ba", frames, poses_est, tracks, pts_O_gt, color_gt, T_OC0_gt, W, H)
