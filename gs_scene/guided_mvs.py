...
class GuidedMVS():
    @torch.no_grad()
    def __init__(self, num_cams, num_depth_candidates=16):
        import os
        self.n_cams = num_cams
        self.num_depth_candidates = num_depth_candidates
        self.idepth_range = 2e-1

        # Read the CUDA source code and set the include directory to current dir
        current_dir = os.path.dirname(__file__)
        with open(os.path.join(current_dir, 'guided_mvs.cu'), 'r') as f:
            cuda_source = f.read()
        ...
        self.module = cupy.RawModule(
            code=cuda_source, 
            options=('--std=c++17', f'-I{current_dir}'),
        )
