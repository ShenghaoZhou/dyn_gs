...
def main(cfg: Config):
    ...
    # Use Gaussian blur to smooth the flow-based s map before gradients?
    s_map_smooth = cv2.GaussianBlur(s_map, (5, 5), 0)
    ...
    # [ (K1_inv [1,0,0]^T)^T ]
    # [ (K1_inv [0,1,0]^T)^T ]   v = [ ds/du ]
    # [ (K1_inv [u,v,1]^T)^T ]       [ ds/dv ]
    #                                [ s     ]
    
    # Pre-calculate rows 1 and 2 (constant for all pixels)
    row_u = K1_inv[0, :] # (K1_inv @ [1,0,0]^T) is the first column. Wait, K1_inv @ [1,0,0]^T is the first column.
    # Row is the transpose.
    m_row1 = K1_inv[:, 0]
    m_row2 = K1_inv[:, 1]
    
    # v = M_inv @ [ds_du, ds_dv, s]^T
    # We can pre-calculate the first two columns of M_inv if possible, but M depends on u, v.
    # However, M is:
    # [ m11 m12 m13 ]  (row_u)
    # [ m21 m22 m23 ]  (row_v)
    # [ x   y   1   ]  (x1^T)
    
    # Actually, let's just solve the 3x3 for each pixel.
    # To do it efficiently in numpy:
    M = np.zeros((H, W, 3, 3))
    M[:, :, 0, :] = m_row1
    M[:, :, 1, :] = m_row2
    M[:, :, 2, :] = x1.reshape(H, W, 3)
    
    rhs = np.stack([ds_du, ds_dv, s_map], axis=-1) # [H, W, 3]
    
    # M @ v = rhs => v = inv(M) @ rhs
    # np.linalg.solve works on stacks
    v_map = np.linalg.solve(M, rhs) # [H, W, 3]
    ...
    n_vis = (n_map + 1.0) / 2.0
    n_vis[~mask_bool] = 0
    rr.log("recovered/normal", rr.Image((n_vis.clip(0, 1) * 255).astype(np.uint8)))
    
    d_vis = d_map[..., 0]
    d_vis[~mask_bool] = 0
    rr.log("recovered/depth", rr.Image(depth_to_rgb(d_vis)))
