...
def main(cfg: Config):
    ...
    prev_f = f0
    for k in range(1, 8): # Use a slightly longer window for better initialization
        ...
        # Simple CV guess for initialization
        T_prev = geo_tracker.poses[fd["frame_idx"]-1]
        if k >= 2:
            T_prev_prev = geo_tracker.poses[fd["frame_idx"]-2]
    ...
    if cfg.run_mapping:
        ...
        mapping_cfg = MappingConfig(device=cfg.device, use_ray_dist=True)
        
        # Correctly initialize GS for mapping from first frame data
        y, x = np.where(prev_f["mask"] > 0)
        ...
        initial_gs_for_mapping = init_gs_from_tracker_points(pts_o, colors0, cfg.device, normals=normals_o, ray_o=ray_o_o, ray_d=ray_d_o, ray_dist=ray_dist)
    ...
    for i, f_curr in tqdm(enumerate(frames_track), total=len(frames_track), desc="Stage 3: Tracking"):
        ...
        # 2. Geometric Step (Refinement)
        # 2a. Projection-based Feature Recovery
        recovered = geo_tracker.match_projections(idx, f_curr["image"], f_curr["mask"], f_curr["K"], T_gs)
        
        # 2b. Compute Prior Flow for DIS
        ...
        # 2c. Tracking Step (Flow + Filtering + PnP)
        success, n_inliers = geo_tracker.step_informed(idx, f_curr["image"], f_curr["mask"], f_curr["depth"], f_curr["K"], T_guess=T_gs, flow_prev_curr=flow, skip_pnp=cfg.skip_pnp)
        if not success:
            print(f"Tracking failed at frame {idx}")
            break
            
        T_CiO = geo_tracker.poses[idx]
        obj_gs.T_W_O = T_CiO # Synchronize GS pose
        
        # 2d. Maintain feature count
        ...
        if is_kf:
            print(f"[Keyframe] Frame {idx}: Disp {avg_disparity:.1f}px, Overlap {overlap_ratio:.2f}, Rec {recovered}, Active {len(active_tids)}")
        ...
        if mapper is not None:
            mapping_data = {
                "image": f_curr["image"],
                "mask": f_curr["mask"],
                "depth": f_curr["depth"] * s_map, # Apply depth scaling
                "K": f_curr["K"],
                "T_CiO": T_CiO,
                "frame_idx": idx
            }
