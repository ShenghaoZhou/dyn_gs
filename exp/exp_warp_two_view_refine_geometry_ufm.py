...
class Config:
    ...
    sample_patches: int = 20000 # Increased for dense per-pixel refinement
    ufm_model: str = "base" # base, refine, base-980, refine-980
    use_covis_weight: bool = True
    use_gt_pose: bool = False # Use GT camera-to-object pose for refinement
...
def patch_warp(H, uv):
    ...
    # Safety: ensure z (depth) is positive to avoid explosion
    z = grid_tmp[..., 2:]
    z_safe = torch.clamp(z, min=1e-3)
    grid = grid_tmp[..., :2] / z_safe
    return grid
...
def optimize_patch_geometry_ufm(image_ref_t, image_tgt_t, n_ref_map, d_ref_map, alpha_ref_map, T_C_O_ref, T_C_O_opt, K, flow_map, covis_map, cfg):
    """
    Refine normal and distance for each 8x8 patch using UFM flow displacement.
    flow_map: [2, H, W] torch tensor
    covis_map: [H, W] torch tensor
    """
    ...
    pbar = tqdm(range(cfg.refine_steps), desc="Refining Geometry (UFM Flow)")
    for step in pbar:
        optimizer.zero_grad()
        
        n_unit = F.normalize(n_param, dim=1)
        # Increase safety margin for distance
        d_safe = torch.clamp(d_param, min=1e-2)
        ...
        # warped_coords: [N, ps*ps, 2]
        # target_coords_flow: [N, ps*ps, 2]
        # sampled_covis: [N, ps*ps]
        # Use Huber loss (per pixel) instead of squared distance to avoid explosion from outliers
        # delta=20 means it becomes linear after 20 pixels of error
    ...
    with torch.no_grad():
        ...
        # This removes the "blocky" artifacts as each pixel is refined independently
        n_refined_map[:, all_patch_centers_y, all_patch_centers_x] = n_final.t()
        d_refined_map[0, all_patch_centers_y, all_patch_centers_x] = d_param.squeeze(-1)
            ...
            rrb.Vertical(
                rrb.Horizontal(
                    ...
                    rrb.Grid(
                        ...
                        rrb.Spatial2DView(name="UFM Flow", contents=["ufm/flow"]),
                        rrb.Spatial2DView(name="UFM Covisibility", contents=["ufm/covisibility"]),
                        name="Optimization"
                    ),
                ),
                name="2D Warping"
...
def main(cfg: Config):
    rr.init("exp_warp_two_view_refine_geometry_ufm", spawn=False)
    ...
    print(f"\nLoading UFM {cfg.ufm_model} model...")
    model_repo_map = {
    ...
    model.to(device)
    model.eval()
    print("UFM Model loaded successfully!")

    print("\nStarting Geometry Refinement with UFM Flow...")
    # Compute UFM flow
    flow_ufm, covis_ufm = predict_ufm_correspondences(model, image_init, image_target_gt)
    
    # Visualize flow
    import flow_vis
    flow_vis_image = flow_vis.flow_to_color(flow_ufm.permute(1, 2, 0).numpy())
    rr.log("ufm/flow", rr.Image(flow_vis_image))
    rr.log("ufm/covisibility", rr.Image(covis_ufm.numpy()))

    # --- CACHE POINT (Up to Step 3: Pose + UFM Flow) ---
    cache_data = {
        "gs_params": {k: v.cpu() for k, v in gs_params.__dict__.items() if isinstance(v, torch.Tensor)},
        "T_C_O_ref": T_C_O_ref_torch.cpu(),
        "T_C_O_est": T_C_O_est.cpu(),
        "flow_ufm": flow_ufm,
        "covis_ufm": covis_ufm,
        "image_init": image_init,
        "image_target_gt": image_target_gt,
        "flow_vis_image": flow_vis_image,
        "T_WC_target_gt": T_WC_target_gt,
        "T_WO_target_gt": T_WO_target_gt,
        "pts_W_gt": pts_W_gt,
        "colors_gt": colors_gt,
        "K": K_torch.cpu(),
        "cfg": cfg
    }
    torch.save(cache_data, "ufm_refine_cache.pth")
    print("\n[Part 1] Cached results to ufm_refine_cache.pth. You can now use the resume script.")
    ...
    n_refined_map, d_refined_map, refine_losses = optimize_patch_geometry_ufm(
        image_ref_t, image_gt_t, n_ref_map, d_ref_map, alpha_ref_map, T_C_O_ref_torch, T_C_O_est, K_torch, flow_ufm, covis_ufm, cfg
    )
    
    # Log refinement loss plot
    fig, ax = plt.subplots(figsize=(4, 3))
    ax.plot(refine_losses)
    ax.set_title("Geometry Refinement Loss (UFM Flow MSE)")
    buf = io.BytesIO()
    fig.savefig(buf, format='png')
    buf.seek(0)
