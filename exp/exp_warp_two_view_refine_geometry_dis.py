...
import time
import cv2  # Ensure cv2 is used for DIS flow
...
def compute_dis_flow(img1, img2):
    """Compute dense optical flow using DIS."""
    if len(img1.shape) == 3:
...
def optimize_patch_geometry_dis(image_ref_t, image_tgt_t, n_ref_map, d_ref_map, alpha_ref_map, T_C_O_ref, T_C_O_opt, K, flow_map, cfg):
    """
    Refine normal and distance for each 8x8 patch using DIS flow displacement.
    flow_map: [H, W, 2] numpy or torch tensor
    """
    ...
    if isinstance(flow_map, np.ndarray):
        flow_map = torch.from_numpy(flow_map).to(device)
    ...
    mask_patches = F.unfold(mask.float().unsqueeze(0).unsqueeze(0), kernel_size=ps, stride=ps)
    coverage = mask_patches.mean(dim=1)[0]
    ...
    # Get target coordinates from DIS flow
    # flow_map is [H, W, 2], we want flow at patch_coords_ref
    # We can use grid_sample to get flow at these positions
    ...
    pbar = tqdm(range(cfg.refine_steps), desc="Refining Geometry (DIS Flow)")
    for step in pbar:
        optimizer.zero_grad()
        ...
        # Loss: flow displacement error
        loss = F.mse_loss(warped_coords, target_coords_flow)
    ...
    with torch.no_grad():
        ...
        for i, idx in enumerate(valid_indices):
            cx, cy = all_patch_centers_x[i].item(), all_patch_centers_y[i].item()
            ...
            rrb.Vertical(
                rrb.Horizontal(
                    ...
                    rrb.Grid(
                        ...
                        rrb.Spatial2DView(name="DIS Flow", contents=["gt/flow"]),
                        name="Optimization"
                    ),
                ),
                name="2D Warping"
...
def main(cfg: Config):
    rr.init("exp_warp_two_view_refine_geometry_dis", spawn=False)
    ...
    print("\nStarting Geometry Refinement with DIS Flow...")
    # Compute DIS flow
    flow_dis = compute_dis_flow(image_init, image_target_gt)
    ...
    flow_vis = flow_to_hsv(flow_dis)
    rr.log("gt/flow", rr.Image(cv2.cvtColor(flow_vis, cv2.COLOR_BGR2RGB)))
    ...
    n_refined_map, d_refined_map, refine_losses = optimize_patch_geometry_dis(
        image_ref_t, image_gt_t, n_ref_map, d_ref_map, alpha_ref_map, T_C_O_ref_torch, T_C_O_est, K_torch, flow_dis, cfg
    )
    
    # Log refinement loss plot
    fig, ax = plt.subplots(figsize=(4, 3))
    ax.plot(refine_losses)
    ax.set_title("Geometry Refinement Loss (DIS Flow MSE)")
    buf = io.BytesIO()
    fig.savefig(buf, format='png')
    buf.seek(0)
