...
class Config:
    ...
    lambda_tv: float = 0.0     # total variation loss on distance
...
def main(cfg: Config):
    ...
    # 4. Optimization
    print("Starting GS optimization (View-Ray Distance)...")
    
    # Calculate view rays in Object Space
    with torch.no_grad():
        T_OC_torch = torch.inverse(T_CO_torch)
        R_OC = T_OC_torch[:3, :3]
        t_OC = T_OC_torch[:3, 3]
        
        # Means in Camera Space
        means_C = (T_CO_torch[:3, :3] @ gs_params.means.T).T + T_CO_torch[:3, 3]
        
        # Distances and Ray Directions in Camera Space
    ...
    distances_anchor = init_distances.clone()
    ...
    optimizer = torch.optim.Adam([
        {'params': [distances], 'lr': cfg.opt_lr},
    ...
    for step in pbar:
        ...
        # Reconstruct means from distances along ray
        ...
        # 4. Total Variation Loss on Distance
        dist_img = torch.zeros((H, W), device=device)
        dist_img[mask_t] = distances
        
        # Horizontal TV
        diff_h = (dist_img[:, 1:] - dist_img[:, :-1]).abs()
        mask_h = mask_t[:, 1:] & mask_t[:, :-1]
        loss_tv_h = diff_h[mask_h].sum()
        
        # Vertical TV
        diff_v = (dist_img[1:, :] - dist_img[:-1, :]).abs()
        mask_v = mask_t[1:, :] & mask_t[:-1, :]
        loss_tv_v = diff_v[mask_v].sum()
        
        loss_tv = (loss_tv_h + loss_tv_v) / (mask_t.sum() + 1e-6)
        
        # 5. Distance Anchor Loss
        loss_dist = F.mse_loss(distances, distances_anchor)
        
        total_loss = cfg.lambda_sv * loss_sv + cfg.lambda_prior * loss_prior + \
                     cfg.lambda_rgb * loss_rgb + cfg.lambda_tv * loss_tv + \
        ...
        pbar.set_description(f"Loss: {total_loss.item():.4f} (SV: {loss_sv.item():.4f}, Prior: {loss_prior.item():.4f}, RGB: {loss_rgb.item():.4f}, TV: {loss_tv.item():.4f}, Dist: {loss_dist.item():.4f})")

    if device.type == "cuda":
        torch.cuda.synchronize()
    end_time = time.time()
    total_time = end_time - start_time
    avg_time = total_time / cfg.opt_steps if cfg.opt_steps > 0 else 0
    ...
    with torch.no_grad():
        means_post = distances.unsqueeze(1) * ray_dirs_O + t_OC.unsqueeze(0)
        img_post, depth_post, normal_post, alpha_post = render_2dgs(
            means_post, quats, scales, colors, opacity, 
            T_CO_torch, K_torch, W, H, cfg.near_plane, cfg.far_plane
        )
        
        pts_O_post = means_post.cpu().numpy()
