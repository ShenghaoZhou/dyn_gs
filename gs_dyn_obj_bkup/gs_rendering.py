...
def render_2dgs(gs_means, gs_rotations, gs_scales, gs_colors, gs_opacity,
    ...
    # render_normal is [3, H, W] in world/object space
    R_cw = viewmat[:3, :3].to(device)
    render_normal = torch.einsum('ij,jhw->ihw', R_cw, render_normal)
...
def render_3dgs(gs_means, gs_rotations, gs_scales, gs_colors, gs_opacity,
    ...
    # Placeholder alpha for 3DGS if not easily accessible
    return color, render_depth, torch.zeros_like(color), torch.ones_like(render_depth) 
