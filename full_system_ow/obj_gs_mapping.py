...
def set_seed(seed: int):
    ...
    # We don't set cudnn.deterministic=True by default as it significantly impacts performance
    # but the above seeds cover most random operations in Python/PyTorch.
...
class GSMapping:
    ...
    def run(self):
        try:
            ...
            if self.cfg.fix_color:
                ...
                if self.gs_params.shs is not None:
                    self.gs_params.shs.requires_grad = False
            else:
                self.gs_params.colors.requires_grad = True
                if self.gs_params.shs is not None:
                    self.gs_params.shs.requires_grad = True
    ...
    def optimize_frame(self, frame_data, num_steps=None, frame_count=0):
        if self.gs_params.means.shape[0] == 0:
            return
        ...
        for step in range(num_steps):
            ...
            if self.cfg.use_pgsr:
                # Only compute SV loss if we have valid normals and depth (2DGS or normal mode)
                if render_normal.any() and render_depth.any():
                    ...
                    if selected_neighbors:
                        ...
                        if len(yy) > 0:
                            ...
                            for kf in selected_neighbors:
                                ...
                                if step >= 60 and render_depth.any():
            ...
            if total_loss.requires_grad:
                ...
                print(f"[Mapper] Warning: total_loss does not require grad at step {step}, skipping backward.")
        ...
        if (frame_count > 0 and frame_count % self.cfg.densify_every == 0) or frame_data.get("force_densify", False):
            before_count = len(self.gs_params.means)
            with self.lock:
                self.densify(image_t, depth, mask_t, cam, T_CO_t)
            after_count = len(self.gs_params.means)
            msg_prefix = "Mapping Frame" if not frame_data.get("force_densify", False) else "Force Densify Frame"
            print(f"{msg_prefix} {frame_count}: Densified {before_count} -> {after_count}")
