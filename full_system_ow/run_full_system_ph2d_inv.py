...
def load_frame_data_ph2d(data_dir, frame_idx):
    ...
    # Extrinsics (Interpreted as Camera-to-World, so we invert for World-to-Camera)
    ext_path = Path(data_dir) / "extrinsics" / f"{frame_idx:04d}.npy"
    if ext_path.exists():
        extrin_raw = np.load(ext_path)
        extrin = np.linalg.inv(extrin_raw)
    else:
        extrin = np.eye(4)
...
def data_loader_worker(cfg, static_q, dynamic_q):
    ...
    for i in range(cfg.num_frames):
        ...
        fd = load_frame_data_ph2d(data_dir, idx)
        if fd is None: 
...
def static_scene_worker(cfg, bg_queue, data_q):
    ...
    data_dir = Path(cfg.data_root) / cfg.clip_id
    f0 = load_frame_data_ph2d(data_dir, cfg.init_frame)
