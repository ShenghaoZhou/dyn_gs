...
@dataclass
class GlobalConfig:
    data_root: str = "data/ph2d_processed"
    clip_id: str = "304-grasp_coke_random-2024_12_12-19_58_36/processed_episode_0"
    device: str = "cuda"
    num_frames: int = 150
    init_frame: int = 0
    n_features: int = 2000
    ...
    use_pgsr: bool = False
    seed: int = 42
    use_model_pose: bool = False

_POSES_CACHE = {}

def load_poses(data_dir):
    if data_dir in _POSES_CACHE:
        return _POSES_CACHE[data_dir]
    poses_file = Path(data_dir) / "poses.json"
    if not poses_file.exists():
        logging.warning(f"Poses file not found: {poses_file}")
        return None
    with open(poses_file, "r") as f:
        data = json.load(f)
    _POSES_CACHE[data_dir] = data["poses"]
    return _POSES_CACHE[data_dir]

def load_frame_data_ph2d(data_dir, frame_idx, use_model_pose=False):
    if frame_idx == 100:
        print(f"[DEBUG] load_frame_data_ph2d frame 100: use_model_pose={use_model_pose}")
    """Helper to load frame data consistent with PH2D conventions"""
    # PH2D naming conventions
    img_path = Path(data_dir) / "images" / "left" / f"{frame_idx:04d}.png"
    if not img_path.exists():
        img_path = Path(data_dir) / "images" / "left" / f"{frame_idx:04d}.jpg"
    
    mask_path = Path(data_dir) / "model_infer" / f"mask_{frame_idx:05d}.png"
    ...
    # K is global in ph2d
    k_path = Path(data_dir).parent.parent / "intrinsics.npy"
    if not k_path.exists():
        k_path = Path(data_dir) / "intrinsics.npy"
    
    if k_path.exists():
        K = np.load(k_path)
        # Check if intrinsics match image resolution
        # If K is for ~1280x720 but image is 640x480, scale K
        if w == 640 and K[0, 2] > 500:
            print(f"[Data Loader] Scaling intrinsics for 640x480 image (assuming crop+resize from 1280x720)")
            # Assuming center crop 960x720 from 1280x720, then resize to 640x480
            # Scale factor = 640/960 = 2/3
            K[0, 0] *= (2.0/3.0)  # fx
            K[1, 1] *= (2.0/3.0)  # fy
            K[0, 2] = (K[0, 2] - 160) * (2.0/3.0)  # cx
            K[1, 2] = K[1, 2] * (2.0/3.0)  # cy
    else:
        # Fallback if not found
        K = np.array([[730.72, 0, 622.29], [0, 730.72, 354.45], [0, 0, 1]])
        if w == 640:
            K[0, 0] *= 0.5; K[1, 1] *= 0.5; K[0, 2] *= 0.5; K[1, 2] *= 0.5
        
    # Extrinsics (World-to-Camera)
    if use_model_pose:
        pose_path = Path(data_dir) / "model_infer" / f"pose_{frame_idx:05d}.npy"
        if pose_path.exists():
            T_WC = np.load(pose_path)
            # Use inversion as requested (Camera-to-World -> World-to-Camera)
            extrin = np.linalg.inv(T_WC)
            if frame_idx % 10 == 0:
                print(f"[DEBUG] Frame {frame_idx}: Loaded from model_infer, extrin[:3,3]={extrin[:3,3]}")
        else:
            extrin = np.eye(4)
    else:
        ext_path = Path(data_dir) / "extrinsics" / f"{frame_idx:04d}.npy"
        if ext_path.exists():
            extrin = np.load(ext_path)
            if frame_idx % 10 == 0:
                print(f"[DEBUG] Frame {frame_idx}: Loaded from extrinsics, extrin[:3,3]={extrin[:3,3]}")
        else:
            extrin = np.eye(4)
        
    # Hand mask (if available, PH2D naming: hand_masks/mask_000xx.png)
    hand_mask_path = Path(data_dir) / "hand_masks" / f"mask_{frame_idx:05d}.png"
    ...
    # Object pose from poses.json (World-from-Object)
    T_WO_gt = None
    all_poses = load_poses(data_dir)
    if all_poses is not None and frame_idx < len(all_poses):
        T_WO_gt = np.array(all_poses[frame_idx]["right_hand_pose"])
...
def data_loader_worker(cfg, static_q, dynamic_q):
    ...
    for i in range(cfg.num_frames):
        ...
        fd = load_frame_data_ph2d(data_dir, idx, use_model_pose=cfg.use_model_pose)
        if fd is None: 
...
def static_scene_worker(cfg, bg_queue, data_q):
    ...
    data_dir = Path(cfg.data_root) / cfg.clip_id
    f0 = load_frame_data_ph2d(data_dir, cfg.init_frame, use_model_pose=cfg.use_model_pose)
...
def main():
    ...
    print(f"[DEBUG] main: cfg.use_model_pose={cfg.use_model_pose}")
    set_seed(cfg.seed)
