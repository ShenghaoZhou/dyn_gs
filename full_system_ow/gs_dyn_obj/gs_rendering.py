...
def render_3dgs(gs_means, gs_rotations, gs_scales, gs_colors, gs_opacity,
    ...
    if shs is not None:
        ...
        # Check if we are in logit space (raw colors) or sigmoid space
    ...
    # Depth estimation from accumulated opacity (simplified)
...
def render_3dgs_full(gs_means, gs_rotations, gs_scales, gs_colors, gs_opacity,
                ...
                far_plane: float = 100.0, scaling_modifier: float = 1.0,
                bg=torch.zeros(3), shs=None):
    """Full rendering for 3DGS, including viewspace statistics for densification."""
    device = gs_means.device
    if gs_means.shape[0] == 0:
        return torch.zeros(3, height, width, device=device), \
               torch.zeros(1, height, width, device=device), \
               ...
               torch.zeros(gs_means.shape[0], device=device), \
               torch.zeros(gs_means.shape[0], 2, device=device)
    ...
    # Extraction for densification
    radii = info["radii"][:, 0] # [N]
    means2D = info["means2d"] # [N, 2]
