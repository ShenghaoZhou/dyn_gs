...
class Config:
    ...
    densify_every: int = 100 # Default to large value to explore its impact
...
def main(cfg: Config):
    ...
    for i in range(cfg.n_frames_track):
        ...
        # After 10 frames, switch to photometric-only tracking by monkey-patching the geometric tracker
        if i >= 10:
            def skip_geo(*args, **kwargs):
                return False, 0
            tracker.tracker.step_informed_with_occlusion = skip_geo
