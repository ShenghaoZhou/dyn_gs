...
def main(cfg: Config):
    if not cfg.no_vis:
        rr.init("BundleGS_HOT3D_Debug", spawn=False)
    ...
    mapping_cfg = MappingConfig(
        gs_type=cfg.gs_type,
        device=device,
        num_steps_per_frame=150,
        densify_every=5 # Explicitly set for debug visibility
    )
    
    # CRITICAL: use_multiprocessing=False for sequential execution
    print("\n[DEBUG] Initializing BundleSdfGS in SEQUENTIAL mode (use_multiprocessing=False)")
    ...
    print("\n>>> Phase 1: Tracking and Mapping (Sequential)")
    for i in range(cfg.n_frames_track):
        ...
        # Initialize in Object space at frame 0
        ...
        if T_CO_gt is not None:
            ...
            if i < 2:
                print(f"[DEBUG] Frame {i} Pose Comparison:")
                print(f"  T_CO_est:\n{T_CO_est}")
                print(f"  T_CO_gt: \n{T_CO_gt}")
    ...
    for abs_idx in pbar:
        ...
        with torch.no_grad():
            ...
            if not cfg.no_vis:
                ...
                if torch.isnan(render_image).any():
                    print(f"[WARNING] NaN detected in render_image at frame {fd['frame_idx']}")
                    render_image = torch.where(torch.isnan(render_image), torch.zeros_like(render_image), render_image)
