...
class Config:
    ...
    output_dir: str = "output/hot3d_log_cam"
...
def main(cfg: Config):
    ...
    if not cfg.no_vis:
        rr.init("BundleGS_HOT3D_LogCam", spawn=False)
        if cfg.rerun_url: rr.connect_grpc(cfg.rerun_url)
        rr.log("object", rr.ViewCoordinates.RIGHT_HAND_Y_DOWN, static=True)
        # Separate sub-paths for persistent keyframes
        rr.log("object/keyframes", rr.ViewCoordinates.RIGHT_HAND_Y_DOWN, static=True)
        rr.log("object/keyframes_gt", rr.ViewCoordinates.RIGHT_HAND_Y_DOWN, static=True)
    ...
    for i in range(cfg.n_frames_track):
        ...
        # Persistent Keyframe Logging
        is_kf_frame = tracker.cnt in tracker.keyframes
        if is_kf_frame:
            print(f"[LogCam] Frame {idx} is a KEYFRAME.")
            if not cfg.no_vis:
                T_OC_est = np.linalg.inv(T_CO_est)
                rr.log(f"object/keyframes/cam_{idx}", rr.Transform3D(mat3x3=T_OC_est[:3, :3], translation=T_OC_est[:3, 3]))
                rr.log(f"object/keyframes/cam_{idx}", rr.Pinhole(image_from_camera=K, width=W, height=H))
                rr.log(f"object/keyframes/cam_{idx}/label", rr.TextDocument(f"KF {idx}"))
                
                if T_CO_gt is not None:
                    T_OC_gt = np.linalg.inv(T_CO_gt)
                    rr.log(f"object/keyframes_gt/cam_{idx}", rr.Transform3D(mat3x3=T_OC_gt[:3, :3], translation=T_OC_gt[:3, 3]))
                    rr.log(f"object/keyframes_gt/cam_{idx}", rr.Pinhole(image_from_camera=K, width=W, height=H))
            
            # Store for JSON (always do this)
            kf_info.append({
                "frame_idx": int(idx),
                "is_kf": True,
                "T_CO_est": T_CO_est.tolist(),
                "T_CO_gt": T_CO_gt.tolist() if T_CO_gt is not None else None,
                "ATE": float(np.linalg.norm(T_CO_est[:3, 3] - T_CO_gt[:3, 3])) if T_CO_gt is not None else None
            })

        # Sliding Window for Rerun Visualization (Show only active keyframes)
        max_active = tracker.tracker_cfg.max_keyframes
        if len(tracker.keyframes) > max_active:
            # Identify the keyframe that just fell out of the window
            old_kf_cnt = tracker.keyframes[-(max_active + 1)]
            abs_old_idx = cfg.init_frame + old_kf_cnt * cfg.stride
            
            if not cfg.no_vis:
                # Clear from Rerun
                rr.log(f"object/keyframes/cam_{abs_old_idx}", rr.Clear(recursive=True))
                rr.log(f"object/keyframes_gt/cam_{abs_old_idx}", rr.Clear(recursive=True))
    ...
    # Save Keyframe JSON
    kf_json_path = os.path.join(cfg.output_dir, f"{cfg.clip_id}_keyframes.json")
    with open(kf_json_path, "w") as f:
        json.dump(kf_info, f, indent=4)
    print(f"[INFO] Saved {len(kf_info)} keyframe poses to {kf_json_path}")

    # Summary Report
    print("\n" + "="*40)
    print(f"Keyframe Statistics for {cfg.clip_id}:")
    print(f"  Total Frames tracked: {cfg.n_frames_track}")
    print(f"  Total Keyframes:      {len(kf_info)}")
    print(f"  Keyframe density:     {len(kf_info)/cfg.n_frames_track*100:.1f}%")
    if all_dist_errors:
        print(f"  Mean ATE:             {np.mean(all_dist_errors):.4f} m")
        print(f"  Max ATE:              {np.max(all_dist_errors):.4f} m")
        print(f"  Mean Rot Error:       {np.mean(all_rot_errors):.2f} deg")
    print("="*40)

    # Phase 2: Evaluation (Optional, but kept for consistency)
    if cfg.n_frames_eval > 0:
        print("\n>>> Phase 2: Evaluation")
        eval_psnrs = []
