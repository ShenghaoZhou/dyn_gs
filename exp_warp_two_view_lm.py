...
class Config:
    ...
    device: str = "cuda"
    opt_steps: int = 25 # Fewer steps for LM
...
def manual_forward_homography_warp(image_ref_t, n_C, d, T_curr_ref, K, mask=None):
    ...
    # Zero out weights for invalid pixels instead of boolean indexing
    v_f = valid.float()
    wa = wa * v_f
    wb = wb * v_f
    wc = wc * v_f
    ...
    # Use index_add (out-of-place) for vmap compatibility
...
def optimize_pose_lm(image_ref_t, gs_params, image_gt_t, K_torch, T_C_O_ref, T_C_O_init, cfg):
    ...
    R_init = init_np[:3, :3]
    t_init = init_np[:3, 3]
    # Scipy as_quat() is [x, y, z, w]. Theseus wants [w, x, y, z]
    ...
    # Move objective to device so it can be evaluated outside
    objective.to(device)
...
def setup_blueprint():
    ...
    blueprint = rrb.Blueprint(
        rrb.Tabs(
            ...
            rrb.Spatial3DView(
                name="Point Clouds",
                contents=["world/object_gt", "world/gs_depth_pc", "world/object_gs", "world/camera_gt", "world/camera_est", "world/camera_init"]
            )
        )
    )
    return blueprint
...
def main(cfg: Config):
    rr.init("exp_warp_two_view_lm", spawn=False)
    ...
    if err_history is not None:
        ...
        with torch.no_grad():
            final_loss = objective.error().norm().item()**2 / 2.0
            losses = [final_loss]
            print(f"Final Loss (manual): {final_loss:.6f}")
    ...
    if err_history is not None:
        ...
        ax.plot(losses)
        ax.set_title("LM Optimization Error")
        buf = io.BytesIO()
    ...
    # Log Init Camera Pose in World (starting guess for frame 40)
